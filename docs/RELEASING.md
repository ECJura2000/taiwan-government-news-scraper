...
cargo run --release --bin news-scraper -- collect --date 2026-09-14 --fail-on-source-error
npm run tauri build
```

Inspect the JSON contract and Excel workbook. Confirm 73 selected sources, no failed source, no quality alert, schema version 4 and the expected relevance hash.

## Publication gate

1. Merge the validated commit to `main`.
2. Create and push immutable tag `v2.1.19`; do not modify any previous tag or Release, including the published rollback `v2.0.0`.
...
7. Run `Release live smoke` for `v2.1.19` and verify all published downloads, including the extracted Windows portable GUI rendered-interface marker.
